...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getCandidateByAddressStmt, err = db.PrepareContext(ctx, getCandidateByAddress); err != nil {
		return nil, fmt.Errorf("error preparing query GetCandidateByAddress: %w", err)
	}
	...
	if q.updateVotedGovernorStmt, err = db.PrepareContext(ctx, updateVotedGovernor); err != nil {
		return nil, fmt.Errorf("error preparing query UpdateVotedGovernor: %w", err)
	}
	if q.updateVotedPresidentStmt, err = db.PrepareContext(ctx, updateVotedPresident); err != nil {
		return nil, fmt.Errorf("error preparing query UpdateVotedPresident: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getCandidateByAddressStmt != nil {
		if cerr := q.getCandidateByAddressStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getCandidateByAddressStmt: %w", cerr)
		}
	}
	...
	if q.updateVotedGovernorStmt != nil {
		if cerr := q.updateVotedGovernorStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing updateVotedGovernorStmt: %w", cerr)
		}
	}
	if q.updateVotedPresidentStmt != nil {
		if cerr := q.updateVotedPresidentStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing updateVotedPresidentStmt: %w", cerr)
		}
	}
...
type Queries struct {
	...
	getCandidateByAddressStmt      *sql.Stmt
	...
	updateVotedGovernorStmt        *sql.Stmt
	updateVotedPresidentStmt       *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		getCandidateByAddressStmt:      q.getCandidateByAddressStmt,
		...
		updateVotedGovernorStmt:        q.updateVotedGovernorStmt,
		updateVotedPresidentStmt:       q.updateVotedPresidentStmt,
